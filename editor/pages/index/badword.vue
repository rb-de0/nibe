        ...
        <template v-slot:body="{ items }">
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>
                <div class="py-2">
                  <template v-show="item.patterns.length > 0" v-for="p in item.patterns">
                    <div :key="p">
                      {{ p }}
                    </div>
                  </template>
...
<script>
import Dialog from '~/components/dialogs/badword-dialog.vue'
...
export default {
  ...
  data() {
    return {
      ...
      pageCount: 0,
      headers: [
        { text: 'パターン', value: 'patterns', width: '40%' },
