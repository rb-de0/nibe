        ...
        <template v-slot:body="{ items }">
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>
                <div class="py-2">
                  <template v-show="item.patternContents.length > 0" v-for="p in item.patternContents">
                    <div :key="p">
                      {{ p }}
                    </div>
                  </template>
                  <div v-if="item.patternContents.length === 0">-</div>
                </div>
              </td>
              <td>
                <div class="py-2">
                  <template v-show="item.synsets.length > 0" v-for="s in item.synsets">
                    <div :key="s">
                      {{ s }}
                    </div>
                  </template>
                  <div v-if="item.synsets.length === 0">-</div>
                </div>
              </td>
              <td>
                <div class="py-2">
                  <template v-show="item.textContains.length > 0" v-for="t in item.textContains">
                    <div :key="t">
                      {{ t }}
                    </div>
                  </template>
                  <div v-if="item.textContains.length === 0">-</div>
                </div>
              </td>
              <td>
                <div class="py-2">
...
<script>
import Dialog from '~/components/dialogs/conversation-dialog.vue'
...
export default {
  ...
  data() {
    return {
      ...
      pageCount: 0,
      headers: [
        { text: 'パターン', value: 'patterns', width: '20%' },
        { text: '概念', value: 'synsets', width: '20%' },
        { text: 'テキスト', value: 'textContains', width: '20%' },
    ...
    this.$axios
      ...
      .then((r) => {
        this.items = r.data.items.map((item) => {
          item.id = uuidv4()
          item.patternContents = item.patterns.map((p) => {
            return p.join(' + ')
          })
          return item
        })
  ...
  methods: {
    ...
    update: function (item, newItem) {
      newItem.id = item.id
      newItem.patternContents = newItem.patterns.map((p) => {
        return p.join(' + ')
      })
